...
<?PHP
$sql="
SELECT MAX(goods_class4_no) FROM `goods_class4` 
";
$result = $db->prepare("$sql");//防sql注入攻擊
$result->execute();
$rows = $result->fetch(PDO::FETCH_ASSOC);

$max_no = $rows["MAX(goods_class4_no)"];//取出資料庫中no的最大值

//跑回圈抓取回傳的編號對應的值

for ($no=1;$no<=$max_no;$no++){

	$goods_class4_sort=$_POST["$no"];

	//echo '編號'.$no.'值等於='.$sort.'<br>';

	$sql_update="UPDATE `goods_class4` SET `goods_class4_sort` = '$goods_class4_sort' WHERE `goods_class4_no` ='$no';";
