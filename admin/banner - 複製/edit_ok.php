...
{//如果有上傳檔案
	...
	//圖片上傳
		...
		$pic_b_del=$rows["banner_pic_b"];
		$pic_s_del=$rows["banner_pic_s"];
	...
	//查詢圖檔		
		...
		$result->execute();
		$rows = $result->fetch(PDO::FETCH_ASSOC);
		$pic_b=$rows["banner_pic_b"];
		$pic_s=$rows["banner_pic_s"];
	}



if($_FILES['imgfile2']['tmp_name']<>"")
{//如果有上傳檔案
	...
	//圖片上傳
		...
		$result->execute();
		$rows = $result->fetch(PDO::FETCH_ASSOC);
		$pic_b_mob_del=$rows["banner_pic_b_mob"];
		$pic_s_mob_del=$rows["banner_pic_s_mob"];
	
		// is_file:判斷檔案是否存在並為正常檔案，傳回布林值
		// unlink:刪除檔案，傳回布林值
		$file="../goods_pic/".$pic_b_mob_del;
		  if(is_file($file)){
			unlink($file);
		  }
		$file="../goods_pic/".$pic_s_mob_del;
		  if(is_file($file)){
			unlink($file);
		  }
		//查詢舊圖檔刪除  
	...
	//查詢圖檔		
		...
		$result->execute();
		$rows = $result->fetch(PDO::FETCH_ASSOC);
		$pic_b_mob=$rows["banner_pic_b_mob"];
		$pic_s_mob=$rows["banner_pic_s_mob"];
	}
...
`banner_pic_b` =:pic_b,
`banner_pic_s` =:pic_s,
`banner_pic_b_mob` =:pic_b_mob,
`banner_pic_s_mob` =:pic_s_mob,
`banner_date` =:edittime
