...
$postwho    =	$_SESSION["id"];//發布者

$news_time  =	date("Y-m-d H:i:s");//新增時間

//發布狀態：有勾選傳回:1
if ( isset($_POST["ckpost"]) && $_POST["ckpost"] != '' ) { $ckpost = '1'; }
else { $ckpost = '0'; }



//發布狀態：無定時發佈時間則傳回:0
if ( isset($_POST["posttime"]) && $_POST["posttime"] != '' ) { $posttime   =	$_POST["posttime"]; }
else { $posttime = '0'; }



//發布狀態：無定時結束時間則傳回:0
if ( isset($_POST["overtime"]) && $_POST["overtime"] != '' ) { $overtime   =	$_POST["overtime"]; }
else { $overtime = '0'; }
...
{//如果有上傳檔案
	...
	//圖片上傳
	    ...
	    $sql="
		SELECT * FROM `news` 
		where news_no=:edit_no";//sql語法
		$result = $db->prepare("$sql");//防sql注入攻擊
		// 數值PDO::PARAM_INT  字串PDO::PARAM_STR
		$result->bindValue(':edit_no', $edit_no, PDO::PARAM_INT);
		$result->execute();
		$rows = $result->fetch(PDO::FETCH_ASSOC);
		$pic_b_del=$rows["news_pic_b"];
		$pic_s_del=$rows["news_pic_s"];
	...
	//查詢圖檔		
		...
		$result->execute();
		$rows = $result->fetch(PDO::FETCH_ASSOC);
		$pic_b=$rows["news_pic_b"];
		$pic_s=$rows["news_pic_s"];
	}
...
`news_postwho` =:postwho,
`news_time`	 =:news_time,
`news_posttime`	 =:posttime,
`news_overtime`	 =:overtime,
`news_ckpost`	 =:ckpost
...
$result = $db->prepare("$sql");//防sql注入攻擊
	...
	$result->bindValue(':postwho', $postwho, PDO::PARAM_STR);
	$result->bindValue(':news_time', $news_time, PDO::PARAM_STR);
	$result->bindValue(':posttime', $posttime, PDO::PARAM_STR);
	$result->bindValue(':overtime', $overtime, PDO::PARAM_STR);
	$result->bindValue(':ckpost', $ckpost, PDO::PARAM_INT);	
