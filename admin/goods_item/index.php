...
$sh_class=$_SESSION["sh_class"];//
	if ($sh_class<>"")
		$sql_where_class= "&& `goods_item_class_main` = '$sh_class'";
	else 
		$sql_where_class= "";
...
$sh_name=$_SESSION["sh_name"];//
	if ($sh_name<>"")
		$sql_where_name= "&& (`goods_item_title` like '%$sh_name%' || `goods_item_model` like '%$sh_name%')";	
	else 
		$sql_where_name= "";
              ...
              <table width="100%" cellpadding="0" cellspacing="0">
                <tr>
                  <td width="1%" align="right" valign="middle" bgcolor="#CDCED1" class="txt_12_gl"><table width="80" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                      <td height="18" align="right" valign="bottom" class="txt_12_gl" style="padding: 0px !important;">品牌：</td>
                    </tr>
                  </table></td>
	  ...
	  while($rows_class = $result_class->fetch(PDO::FETCH_ASSOC)) 
		{
			...
			if ($_SESSION["sh_class"]==$rows_class["goods_class_main_no"]){
				echo ' selected';
				}			
...
?>
                  ...
                  <td width="1%" valign="middle" bgcolor="#CDCED1" class="txt_12_gl"><input name="sh_name" type="text" id="sh_name" style="width:200px;" value="<?PHP echo $_SESSION["sh_name"]; ?>" Placeholder=" 商品名稱&型號"></td>
			...
			<table width="100%" cellpadding="1" cellspacing="1" class="table_01 stripe">
                ...
                <tbody>
                  ...
                  <tr>				  
                    ...
                    <th width="6%" align="center" bgcolor="#367FA9" class="txt_wt">圖片</th>

                    <th width="7%" align="center" bgcolor="#367FA9" class="txt_wt"><strong>食品</strong></th>
                    <th width="7%" align="center" bgcolor="#367FA9" class="txt_wt">廠牌</th>
                    <th width="7%" align="center" bgcolor="#367FA9" class="txt_wt">設備(產線)</th>
                    <th width="9%" align="center" bgcolor="#367FA9" class="txt_wt">設備(單機)</th>
                    <th width="9%" align="center" bgcolor="#367FA9" class="txt_wt">其他</th>

                    <th height="25" align="center" bgcolor="#367FA9" class="txt_wt">商品品牌/名稱/型號</th>
...
$sql="
SELECT goods_item.*,goods_class_main.*
FROM goods_item 
JOIN goods_class_main 
ON goods_item.goods_item_class_main =goods_class_main.goods_class_main_no
WHERE goods_item_no<>''
$sql_where_class $sql_where_name 
ORDER BY  `goods_item_sort` ASC, `goods_item_no` DESC 
...
?>
                  ...
                  <tr>
                    ...
                    <td height="25" align="center" nowrap="nowrap" bgcolor="#FFFFFF"  class="txt_12_gl"><span class="text_item" style="text-indent:5px;">
                     <img src="../goods_pic/<?=$rows["goods_item_pic_s"]; ?>" height="50" border="1" style=" border-color:#666666; border-style:solid;" onerror="this.src='../goods_pic/defpic.jpg'" >
                    </span></td>
	...
	?>
						<span class="well" style="padding: 5px;"><?=$rows_class["goods_class_name"];?></span>
	<?PHP
		}					
	}
	?> 
	 ...
	 while($rows_class = $result_class->fetch(PDO::FETCH_ASSOC)) 
	{
		...
		if (strpos($string, $find) !== false) {
	?>
						<span class="well" style="padding: 5px;"><?=$rows_class["goods_class2_name"];?></span>
	<?PHP
		}					
	}
	?> 
	 ...
	 while($rows_class = $result_class->fetch(PDO::FETCH_ASSOC)) 
	{
		...
		if (strpos($string, $find) !== false) {
	?>
						<span class="well" style="padding: 5px;"><?=$rows_class["goods_class3_name"];?></span>
	<?PHP
		}					
	}
	?> 
	 ...
	 while($rows_class = $result_class->fetch(PDO::FETCH_ASSOC)) 
	{
		...
		if (strpos($string, $find) !== false) {
	?>
						<span class="well" style="padding: 5px;"><?=$rows_class["goods_class4_name"];?></span>
	<?PHP
		}					
	}
	?> 
	 ...
	 while($rows_class = $result_class->fetch(PDO::FETCH_ASSOC)) 
	{
		...
		if (strpos($string, $find) !== false) {
	?>
						<span class="well" style="padding: 5px;"><?=$rows_class["goods_class5_name"];?></span>
	<?PHP
		}					
	}
	?> 
                    ...
                    <td height="25" align="left" bgcolor="#FFFFFF" class="txt_12_gl" style="overflow:hidden;text-overflow:ellipsis;">
                      <span class="well" style="padding: 5px;"><?=$rows["goods_class_main_name"]; ?></span>  <?=$rows["goods_item_model"]; ?> </br><?=$rows["goods_item_title"]; ?>
                    </td>

                    <td height="25" align="center" bgcolor="#FFFFFF" class="txt_12_gl">
                      <?PHP
						if($rows["goods_item_hide"]==0)
                    ...
                    <td height="25" align="center" nowrap="nowrap" bgcolor="#FFFFFF" class="txt_12_gl">
                      <input type="hidden" name="ck_<?=$rows["goods_item_no"];?>" id="ck_<?=$rows["goods_item_no"];?>" value="<?=$rows["goods_item_sort"];?>">
                      <input name="<?=$rows["goods_item_no"];?>" type="text" id="<?=$rows["goods_item_no"];?>" style="width:40px;" value="<?=$rows["goods_item_sort"];?>">                    </td>
                    <td height="25" align="center" nowrap="nowrap" bgcolor="#FFFFFF" class="txt_12_gl"><a href="edit.php?no=<?=$rows["goods_item_no"];?>"  class="a_movetop"><span class="label label-success"><i class="fa fa-edit"></i> 修改</span></a></td>

                    <td height="25" align="center"nowrap="nowrap" bgcolor="#FFFFFF" class="txt_12_gl">
                    
                    <span class="label label-danger a_movetop" onclick ="return del_<?=$no_id?>()"  ><i class="fa fa-trash"></i> 刪除</span>
                      <input id="link_<?=$no_id?>" type="hidden" value="del_ok.php?no=<?=$rows["goods_item_no"];?>&bpic=<?=$rows["goods_item_pic_b"];?>&spic=<?=$rows["goods_item_pic_s"];?>"  />
