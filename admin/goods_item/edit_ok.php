...
//class
$class = $_POST['class'];
  foreach ($class as $class_tmp) {
    //echo $class_tmp . "<br>";
	  $class_txt = $class_txt.','. $class_tmp;
  }
$class_txt=$class_txt.',';
//class
...
//class2
$class2 = $_POST['class2'];
  foreach ($class2 as $class2_tmp) {
    //echo $class_tmp . "<br>";
	  $class2_txt = $class2_txt.','. $class2_tmp;
  }
$class2_txt=$class2_txt.',';
//class2

//class3
$class3 = $_POST['class3'];
  foreach ($class3 as $class3_tmp) {
    //echo $class_tmp . "<br>";
	  $class3_txt = $class3_txt.','. $class3_tmp;
  }
$class3_txt=$class3_txt.',';
//class3

//class4
$class4 = $_POST['class4'];
  foreach ($class4 as $class4_tmp) {
    //echo $class_tmp . "<br>";
	  $class4_txt = $class4_txt.','. $class4_tmp;
  }
$class4_txt=$class4_txt.',';
//class4

//class5
$class5 = $_POST['class5'];
  foreach ($class5 as $class5_tmp) {
    //echo $class_tmp . "<br>";
	  $class5_txt = $class5_txt.','. $class5_tmp;
  }
$class5_txt=$class5_txt.',';
//class5
...
{//如果有上傳檔案
	...
	//圖片上傳
		...
		where goods_item_no=:edit_no";//sql語法
		$result = $db->prepare("$sql");//防sql注入攻擊
		// 數值PDO::PARAM_INT  字串PDO::PARAM_STR
		$result->bindValue(':edit_no', $edit_no, PDO::PARAM_INT);
		$result->execute();
		$rows = $result->fetch(PDO::FETCH_ASSOC);
		$pic_b_del=$rows["goods_item_pic_b"];
		$pic_s_del=$rows["goods_item_pic_s"];
	...
	//查詢圖檔		
		...
		$result->execute();
		$rows = $result->fetch(PDO::FETCH_ASSOC);
		$pic_b=$rows["goods_item_pic_b"];
		$pic_s=$rows["goods_item_pic_s"];
	}
...
`goods_item_title` =:title,
`goods_item_model` =:model,
`goods_item_class_main` =:class_main,
`goods_item_class` =:class_txt,
`goods_item_class2` =:class2_txt,
`goods_item_class3` =:class3_txt,
`goods_item_class4` =:class4_txt,
`goods_item_class5` =:class5_txt,
`goods_item_pic_s` =:pic_s,
`goods_item_pic_b` =:pic_b,
`goods_item_hide` =:hide,
`goods_item_description` =:description,
`goods_item_content` =:content,
`goods_item_edituser` =:edituser,
`goods_item_edittime` =:edittime
WHERE `goods_item_no` =:edit_no ;";

$result = $db->prepare("$sql");//防sql注入攻擊
	// 數值PDO::PARAM_INT  字串PDO::PARAM_STR
	$result->bindValue(':title', $title, PDO::PARAM_STR);
	$result->bindValue(':class_main', $class_main, PDO::PARAM_INT);
	$result->bindValue(':class_txt', $class_txt, PDO::PARAM_INT);
	$result->bindValue(':class2_txt', $class2_txt, PDO::PARAM_INT);
	$result->bindValue(':class3_txt', $class3_txt, PDO::PARAM_INT);
	$result->bindValue(':class4_txt', $class4_txt, PDO::PARAM_INT);
	$result->bindValue(':class5_txt', $class5_txt, PDO::PARAM_INT);
